...
const LetterSection: React.FC = () => {
  ...
  return (
    <section
      className="py-24 bg-surface flex flex-col items-center justify-center overflow-hidden"
      role="region"
      aria-label="Carta interactiva para los novios"
    >
      <div className="relative w-full max-w-[450px] aspect-4/5 flex items-center justify-center perspective-1000">
          ...
          {isOpen && (
            ...
            >
              <div className="w-full text-right mb-4" aria-live="polite">
                <span
                  className="text-xs font-sans font-bold text-primary-custom/40 tracking-tighter"
                  aria-label={`Mensaje ${currentIndex + 1} de ${messages.length}`}
                >
              ...
              <h3
                id="letter-title"
                className="font-serif italic text-2xl text-on-surface-custom mb-8"
              >
              ...
              {currentIndex === messages.length - 1 ? (
                ...
                <div
                  ...
                  aria-hidden="true"
                >
                  <span className="text-[10px] uppercase tracking-widest text-primary-custom/30 font-bold">
                    Deslizar para seguir leyendo
                  </span>
                  <motion.div
                    animate={{ x: [0, 10, 0] }}
        ...
        <motion.div
          ...
          aria-label={
            isOpen
              ? "Imagen de portada desplazada"
              : "Tocar para abrir carta interactiva"
          }
        >
          <div
            className="absolute inset-0 bg-black/10 transition-opacity group-hover:opacity-0"
            aria-hidden="true"
          />
          <Image
            src="/boda/rosa.jpg"
          ...
          {!isOpen && (
            ...
            >
              <div className="bg-white/90 backdrop-blur-md px-6 py-3 rounded-full shadow-lg">
                <span className="font-serif text-primary-custom italic text-sm">
                  Tocar para leer
                </span>
              </div>
            </div>
